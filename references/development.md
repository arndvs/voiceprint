...
- Deterministic work goes in `scripts/` (Python)
- Non-deterministic work (writeprint, excerpt analysis, SKILL.md writing) is the agent's job
- State persists in `.run/state.json` (gitignored). Every phase must persist status via `self.state`
- The templates in `templates/` are the contract — `assemble` validates placeholders against them
